...
function buildSearchTokens({ itemName, variationName, sku, gtin }) {
  ...
  const addText = (txt) => {
    const t = s(txt);
    if (!t) return;
    t.split(/[^a-z0-9]+/g).filter(Boolean).forEach(w => tokens.add(w));
  };

  addText(itemName);
  addText(variationName);
  addText(sku);
  addText(gtin);

  // capture things like "375 ml", "375ml", "50ml"
  const combined = `${s(itemName)} ${s(variationName)}`;
  const re = /(\d+(?:\.\d+)?)\s*(ml|l|oz)\b/g;
  let m;
  while ((m = re.exec(combined)) !== null) {
    const num = m[1];
    const unit = m[2];
    tokens.add(`${num}${unit}`);   // 375ml
    tokens.add(`${num}`);         // 375 (optional but helpful)
  }
  ...
  for (const loc of locations) {
    ...
    do {
      ...
      for (const c of counts) {
        ...
        if (IGNORE_STATES.has(c.state)) {
          // If you want to actively remove these docs:
          ...
          const merchantInvRef = firestore
            ...
            .collection('inventory').doc(docId);

          batch.delete(masterRef); writes++;
          batch.delete(merchantInvRef); writes++;

          if (writes >= MAX_WRITES_PER_BATCH) await flush();
          continue;
        }
        ...
        const gtin_raw = variation?.itemVariationData?.upc || null;
        const gtin = gtin_raw ? canonicalGtin(gtin_raw) : null;
    ...
    console.log(`Finished location ${loc.id} for merchant ${merchantId}. Total counts fetched=${totalCounts}`);
  }
